...
var (
	...
	ErrLockingBalance = errors.New("locking 1000 emtc")


	ErrLowBalance = errors.New("the balance cannot be less than 1000")
)
...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	/*if pool.chain.CurrentBlock().DposContext.CandidateTrie().Get(from.Bytes()) != nil{
		if pool.currentState.GetBalance(from).Cmp(new(big.Int).Add(tx.Cost(),num)) < 0{

			return ErrLockingBalance
		}
	}*/
	if tx.Type() == 1 {
		if pool.currentState.GetBalance(from).Cmp(num) < 0{
